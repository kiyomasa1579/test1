...
window.onload = function () {
	...
	//BGM読み込み
	const BGMUrl = "n114.mp3";				//game.htmlからの相対パス
	game.preload([BGMUrl]); 				//データを読み込んでおく

	//背景読み込み
	const backgroundURL = "orange.png";						//game.htmlからの相対パス
	game.preload([backgroundURL]); 				//データを読み込んでおく
	...
	game.onload = function () {					//ロードが終わった後にこの関数が呼び出されるので、この関数内にゲームのプログラムを書こう
		...
		// startScene.backgroundColor = "blue";
		// startScene.backgroundImage = "zoyama.png";
		// startScene.Sprite = "orange.png";


		
		const background = new Sprite(800, 700);				//画像サイズをここに書く。使う予定の画像サイズはプロパティで見ておくこと
		background.moveTo(0, 0);						//リトライボタンの位置
		background.image = game.assets[backgroundURL];			//読み込む画像の相対パスを指定。　事前にgame.preloadしてないと呼び出せない
		startScene.addChild(background);

		//スタート文字
		const startText = new Label(); 					//テキストはLabelクラス
		startText.font = "20px Meiryo";				//フォントはメイリオ 20px 変えたかったらググってくれ
		startText.color = 'rgba(255,255,255,1)';		//色　RGB+透明度　今回は白
		startText.width = 300;							//横幅指定　今回画面サイズ400pxなので、width:400pxだと折り返して二行目表示してくれる
		startText.moveTo(0, 30);						//移動位置指定
		startText.text="ボタン押してスタート"
		startScene.addChild(startText);						//endSceneシーンにこの画像を埋め込む



		//スタートボタン
		const startBtn = new Sprite(120, 60);				//画像サイズをここに書く。使う予定の画像サイズはプロパティで見ておくこと
		startBtn.moveTo(150, 300);						//リトライボタンの位置
		startBtn.image = game.assets[retryImgUrl];			//読み込む画像の相対パスを指定。　事前にgame.preloadしてないと呼び出せない
		startScene.addChild(startBtn);					//endSceneにこのリトライボタン画像を貼り付ける

		startBtn.ontouchend = function () {				//S_Retryボタンをタッチした（タッチして離した）時にこの中の内容を実行する
			state = 0;
		...
		const mainbackground = new Sprite(800, 700);				//画像サイズをここに書く。使う予定の画像サイズはプロパティで見ておくこと
		mainbackground.moveTo(0, 0);						//リトライボタンの位置
		mainbackground.image = game.assets[backgroundURL];			//読み込む画像の相対パスを指定。　事前にgame.preloadしてないと呼び出せない
		mainScene.addChild(mainbackground);
		...
		zoyamaImg.moveTo(118, 100);						//キャラの位置
